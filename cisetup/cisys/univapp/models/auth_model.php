...
/*
 * Auth is used to manage a database of users on your website. Can protect certain pages of your website, giving users access keys to
 * the different portions of the website. Users can also have administrator access and universal keys to gain access to any section on
 * the website.
 *
 * ***NOTE:
 * In order to create a very secure password storage system, you can create a register_date for each user, which will be unique for
 * each user and invisible to website visitors, and use that as the salt for passwords.
 */
...
class Auth_Model extends Model {
	...
	var $program = 'univapp';
	//	program name (for example, ezsell, ezstore, user-defined program)
	var $protected_pages = array();
	//	pages to protect along with access level
	var $roles = array('user', 'admin');
	//  Roles to be used on website
	...
	/*
	 * Auto-login is used for convenience and to allow the user to choose to save login data or not. To enhance security, enable cookie
	 * encryption and set a very unique encryption key.
	 */
	function auto_login() {
		if (empty($this -> user)) {
			$cookie_hash = get_cookie('ezauth_user_info', TRUE);
			if (!empty($cookie_hash))
				$result = $this -> login(null, null, true, $cookie_hash);
			if (!empty($result))
	...
	/*
	 * Establish user session if login information correct. Call login() and use default POST username and POST password or use
	 * parameters login('username here', 'password here');.  Can also login from cookie hash.
	 */
	...
	function login($un = null, $pw = null, $give_new_key = false, $cookie_hash = '', $get_all_user_info = true) {
		...
		if ($query -> num_rows() > 0) {
			...
			if ($userdata -> activation_code != $v_code)
				return array('authorize' => false, 'error' => 'Account not active.', 'code' => 'not_active');
	...
	function new_access_key($user_id, $program, $access = 'user') {
		$inp = array('user_id' => $user_id, 'program' => $program, 'access' => $access, );
	...
	/*
	 * Authorize current user for page specified, Returns 'yes' or 'no' with error message
	 */
	...
	function authorize($page, $give_new_key = false) {
		...
		if (!array_key_exists($page, $pp)) {
			if (!array_key_exists('all', $pp)) {
				...
				if ($program_key == 'user' || $program_key == 'admin')
					return $auth_ok;
				else
					return $auth_not_ok;
			}
	...
	// Activation keys are generated by user information and a timestamp, and are only enabled after a user verifies their e-mail address.
	...
	function register($inp, $verify = true, $remember_user = false, $login_after_register = true) {
		if (empty($inp['ez_users']['username']))
			return array('reg_ok' => 'no', 'error' => 'Username required.');
		if (empty($inp['ez_users']['email']) && $verify == true)
			return array('reg_ok' => 'no', 'error' => 'E-mail required for e-mail validation.');
		...
		//	Convert string to lower case before finding duplicates in database keeps original data as is
		...
		if ($query -> num_rows() > 0)
			return array('reg_ok' => 'no', 'error' => 'Username already exists.');
		...
		$inp3 = array('user_id' => $user_id, 'password' => $this -> _add_salt($pw), 'cookie_hash' => $cookie_hash);

		//	For e-mail verification, If verification is true, set to random code, if not, set to pre-determined code that will be used
		//  for authentication when logging in
		if ($verify == true) {
			$v_code = md5(md5(microtime()) . md5($this -> random_string()));
			//	temporary access code that will be sent to e-mail for verification
		...
		if ($remember_user) {
			...
			$cookie = array('name' => 'user_info', 'value' => $cookie_hash, 'expire' => $this -> cookie_expire, 'domain' => $this -> cookie_domain, 'path' => '/', 'prefix' => 'ezauth_');
			set_cookie($cookie);
		}
	 ...
	 * On new login after verification setting, GIVE NEW KEY IS SET TO FALSE SO A KEY MUST BE ALREADY SET FOR CURRENT PROGRAM
	 * IF AUTO-LOGIN IS SET TO TRUE
	 */
	function verify_email($code, $login_after_verify = false) {
		...
		if ($query -> num_rows() == 1) {
			...
			if ($login_after_verify)
				return $this -> login(null, null, false, $user -> cookie_hash);
			//	login by cookie hash
			else
				return true;
		} else {
			return false;
	...
	function get_userid($username = '', $email = '') {
		...
		if (!empty($userdata))
			$user = array('user_id' => $userdata -> id, 'active' => $active);
		else
			$user = false;
	...
	//	If you use functions that remember the user, you must have cookie helper enabled!!
	...
	function remember_user($user = null, $reset_cookie = false) {
		...
		//	No cookie hash found, so make new one and save or if reset cookie is specified
		...
		//	set cookie with very unique cookie hash
		$cookie = array('name' => 'user_info', 'value' => $db_user -> cookie_hash, 'expire' => $this -> cookie_expire, 'domain' => $this -> cookie_domain, 'path' => '/', 'prefix' => 'ezauth_');
		set_cookie($cookie);

		return true;
	}

	//	Gets all user info in ez_users table based on hash saved in cookie. Runs cookie through XSS filter
